...
export function registerAllTools(server: McpServer): void {
  ...
  server.registerTool(
    'analyze_screenshot',
    {
      description: 'Ingest a base64 screenshot, check visual state cache, and return details of the state (generating a new memory entry if not matched).',
      inputSchema: z.object({
        screenshot: z.string().describe('Base64-encoded image string (required)'),
        description: z.string().optional().describe('Optional human/agent-provided description of the screen'),
        accessibility_tree: z.string().optional().describe('Optional JSON string representing the simplified AX tree'),
      ...
      try {
        ...
        if (retrieval.is_known && retrieval.state_id) {
          ...
          if (params.description && params.description !== retrieval.description) {
            await storage.updateState(retrieval.state_id, { description: params.description });
            retrieval.description = params.description;
          }
          return {
            content: [{ type: 'text', text: JSON.stringify(retrieval, null, 2) }],
          };
        }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to analyze screenshot: ${error.message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    'recall_memory',
    {
      description: 'Search visual memory by text description query or image query (or both) to locate relevant past states.',
      inputSchema: z.object({
        query: z.string().optional().describe('Text search description (e.g. "error dialog")'),
        screenshot: z.string().optional().describe('Base64 image to search visually'),
        strategy: z.enum(['fast', 'semantic', 'thorough']).optional().describe('Matching depth strategy'),
        limit: z.number().int().min(1).max(10).optional().describe('Maximum number of results to return'),
        include_transitions: z.boolean().optional().describe('Include outbound transitions in results'),
        git_branch: z.string().optional().describe('Override the active git branch'),
      }),
    },
    async (params) => {
      try {
        ...
        if (retrieval.state_id) {
          ...
          if (params.include_transitions) {
            transitions = await storage.listTransitions(`from_state_id = '${retrieval.state_id}'`);
          }
          ...
          results.push({
            ...
            transitions: transitions.map(t => ({
              ...
              success_rate: (t.success_count + t.failure_count) > 0 
        ...
        if (retrieval.related_states) {
          for (const item of retrieval.related_states.slice(0, limit - results.length)) {
            let transitions: any[] = [];
            if (params.include_transitions) {
              transitions = await storage.listTransitions(`from_state_id = '${item.id}'`);
            }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to recall memory: ${error.message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    'record_outcome',
    {
      description: 'Record an action outcome and transition between UI states to build the navigation graph.',
      inputSchema: z.object({
        from_state_id: z.string().describe('ID of the starting visual state'),
        to_state_id: z.string().optional().describe('ID of the resulting visual state'),
        to_screenshot: z.string().optional().describe('Base64 image of state after action (auto-analyzed if provided)'),
        action: z.string().describe('Action text performed (e.g. "click submit")'),
        action_type: z.enum(['click', 'type', 'navigate', 'scroll', 'custom']).optional().describe('Type of action'),
    ...
    async (params) => {
      try {
        ...
        // If to_screenshot is provided, ingest it first to resolve resulting ID
        if (params.to_screenshot) {
          const buffer = Buffer.from(params.to_screenshot, 'base64');
        ...
        if (!toStateId) {
          throw new Error('Either to_state_id or to_screenshot must be provided.');
        }
        ...
        const totalAttempts = transition.success_count + transition.failure_count;
        const successRate = totalAttempts > 0 ? transition.success_count / totalAttempts : 0;
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to record outcome: ${error.message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    ...
    {
      description: 'Trace historical pathways from current state to a target state or state matching description.',
      inputSchema: z.object({
        from_state_id: z.string().optional().describe('ID of starting state (defaults to most recently accessed state)'),
    ...
    async (params) => {
      try {
        ...
        if (!fromId) {
          ...
          const states = await storage.listStates(`git_branch = '${branch}'`, 1);
          if (states.length > 0) {
            fromId = states[0].id;
          }
        }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to get navigation paths: ${error.message}` }],
        };
      }
    }
  );
    ...
    async (params) => {
      try {
        ...
        const result = {
          ...
          structural_diff: stateA.structured_data === stateB.structured_data 
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to compare states: ${error.message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    'get_session_context',
    {
      description: 'Fetch aggregated visual context, listing recent/frequent states and active transitions.',
      inputSchema: z.object({
        include_recent: z.number().int().min(1).max(20).optional().describe('Count of recent states'),
        include_frequent: z.number().int().min(1).max(20).optional().describe('Count of frequent states'),
      }),
    },
    async (params) => {
      try {
        ...
        const recentList = await storage.listStates(`git_branch = '${branch}'`, 100);
        ...
        // 3. Transitions
        const transitions = await storage.listTransitions(`git_branch = '${branch}'`, 50);
        ...
        // 4. Memory stats
        const allStatesCount = (await storage.listStates(undefined, 10000)).length;
        const allTransCount = (await storage.listTransitions(undefined, 10000)).length;
        ...
        const result = {
          ...
          memory_stats: {
            ...
            db_size_mb: 0.1, // estimate or read dir size
          },
        };
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to get session context: ${error.message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    ...
    async (params) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to save visual snapshot: ${error.message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    ...
    {
      description: 'Diff two snapshots to locate additions, deletions, or visual drift regressions.',
      inputSchema: z.object({
    ...
    async (params) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Failed to diff snapshots: ${error.message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    'undo_last_visual_mutation',
    {
      ...
      inputSchema: z.object({
        type: z.enum(['state', 'transition', 'any']).optional().describe('Type of mutation to revert'),
      }),
    },
    async (params) => {
      try {
        ...
        const undoTransition = async (): Promise<boolean> => {
          const list = await storage.listTransitions(`git_branch = '${branch}'`, 100);
          if (list.length === 0) return false;
        ...
        if (type === 'state') {
          ...
          const stateList = await storage.listStates(`git_branch = '${branch}'`, 1);
          const transList = await storage.listTransitions(`git_branch = '${branch}'`, 1);
