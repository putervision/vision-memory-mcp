...
/**
 * Converts Zod schemas or definitions to JSON Schema object
 */
export function zodToJsonSchema(zodSchema: any): Record<string, any> {
  if (!zodSchema) return { type: 'object' };
  if (typeof zodSchema.toJsonSchema === 'function') {
    return zodSchema.toJsonSchema();
  }
  if (!zodSchema._def) {
    if (typeof zodSchema === 'object' && (zodSchema.type || zodSchema.properties)) {
      return zodSchema;
    }
    return { type: 'object' };
  }

  function parseDef(def: any): Record<string, any> {
    if (!def) return {};
    const typeName = def.typeName;
    const desc = def.description;

    if (typeName === 'ZodOptional' || typeName === 'ZodNullable') {
      return parseDef(def.innerType?._def);
    }
    if (typeName === 'ZodDefault') {
      return parseDef(def.innerType?._def);
    }
    if (typeName === 'ZodEffects') {
      return parseDef(def.schema?._def);
    }
    ...
    switch (typeName) {
      ...
      case 'ZodArray':
        out.type = 'array';
        if (def.type?._def) {
          out.items = parseDef(def.type._def);
        }
        break;
      case 'ZodObject': {
        out.type = 'object';
        ...
        if (shape) {
          for (const [key, field] of Object.entries(shape)) {
            ...
            properties[key] = parseDef(fieldAny?._def);
            if (fieldAny?.description && !properties[key].description) {
              properties[key].description = fieldAny.description;
            }

            // Determine if property is optional
            let cur = fieldAny?._def;
            let isOptional = false;
            while (cur) {
              if (cur.typeName === 'ZodOptional' || cur.typeName === 'ZodDefault') {
                isOptional = true;
                break;
              }
              if (cur.typeName === 'ZodEffects') {
                cur = cur.schema?._def;
              } else if (cur.typeName === 'ZodNullable') {
  ...
  return parseDef(zodSchema._def);
}
...
export class NativeMcpServer {
  ...
  registerResource(
    ...
    if (typeof uriOrTemplate === 'string') {
      ...
      templateStr = uriOrTemplate.uriTemplate;
    } else if (typeof uriOrTemplate?.uriTemplate?.template === 'string') {
      templateStr = uriOrTemplate.uriTemplate.template;
    } else if (typeof uriOrTemplate?._uriTemplate?.template === 'string') {
    ...
    if (!templateStr.includes('{')) {
      this.resources.set(templateStr, {
        name,
        uri: templateStr,
        title: metadata.title,
        description: metadata.description,
          ...
          prompts: Array.from(this.prompts.values()).map((p) => {
            ...
            if (p.argsSchema?.properties) {
              ...
              args = Object.entries(p.argsSchema).map(([k, v]: [string, any]) => ({
                name: k,
                description: v?.description || (v as any)?._def?.description,
                required: !(v as any)?._def?.typeName?.includes?.('Optional'),
              }));
            }
            return {
              name: p.name,
  ...
  get _registeredTools(): Record<string, any> {
    const res: Record<string, any> = {};
