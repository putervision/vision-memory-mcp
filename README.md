...
```
This scaffolds `./data/vision-memory`, generates a default `.env` template, and adds configuration properties to your `.gitignore`.
...
```json
{
  "mcpServers": {
    "vision-memory-mcp": {
      ...
      "args": ["run"],
      "env": {
        "LANCEDB_PATH": "./data/vision-memory"
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "vision-memory-mcp": {
      ...
      "args": ["run"],
      "env": {
        "LANCEDB_PATH": "/absolute/path/to/your/data/vision-memory"
      }
    }
  }
}
...
```bash
LANCEDB_PATH=./data/vision-memory        # Storage path for LanceDB
