...
This project uses `world-model-mcp` with project slug "vision-memory-mcp" to maintain a persistent 3D/2D spatial internal world model, entity tracking, object permanence, and movement simulation.
...
This project uses `agent-reasoning-mcp` with project slug "vision-memory-mcp" to manage goals, decompose complex tasks, evaluate situational trade-offs, and track decision rationale.
...
This project uses `behavior-mcp` with project slug "vision-memory-mcp" to execute deterministic behavior trees at ~60Hz in browser runtimes with reactive triggers and safety guardrails.
